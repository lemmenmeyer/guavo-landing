...
function fcraBlockHtml({ includeScore, score, scoreDate, keyFactors }) {
  ...
  const scoreLine = includeScore ? `
      <p style="margin:10px 0 0;">The credit score used was <strong>${escapeHtml(score)}</strong> (${EXPERIAN.name}, ${escapeHtml(scoreDate)}). FICO scores range from 300 to 850. Key factors: ${factors}.</p>` : '';
  ...
  return `
    <div style="background:${PALETTE.fcraStripe};border-left:3px solid ${PALETTE.fcraBorder};padding:14px 16px;margin:0 0 18px;border-radius:3px;font-size:13px;color:${PALETTE.ink};">
      <p style="margin:0;">Our decision was based in part on your consumer report from ${EXPERIAN.name} (${EXPERIAN.addr1}, ${EXPERIAN.city}, ${EXPERIAN.state} ${EXPERIAN.zip}; ${EXPERIAN.phone}; ${EXPERIAN.web}). ${EXPERIAN.name} did not make this decision. You have the right to a free copy of your report from ${EXPERIAN.name} within 60 days and to dispute any inaccurate information it contains.</p>${scoreLine}
    </div>
  `;
}
...
function fcraBlockText({ includeScore, score, scoreDate, keyFactors }) {
  ...
  return `${base}\n\nThe credit score used was ${score} (${EXPERIAN.name}, ${scoreDate}). FICO scores range from 300 to 850. Key factors: ${factors}.`;
}

// Join threshold clauses naturally with "and". Handles 1, 2, or more.
//
// Dedupes first: more than one reason can legitimately resolve to the same
// threshold string, because fraudulent-documents.js and manufactured-deposits.js
// both delegate to under-minimum-revenue.js so the floor lives in one place.
// Without this, pairing two of them emitted "an average monthly revenue of at
// least $15,000 and an average monthly revenue of at least $15,000."
function joinThresholds(list) {
  const arr = [...new Set(list.filter(Boolean).map(s => String(s).trim()))];
...
function composeDeclineEmail({
  ...
  // Split reasons into two kinds. "Improvable" reasons (revenue, ADB, FICO)
  // are bars the applicant can clear over time, so they fold into the
  // "We generally look for X" clause. "Coverage" reasons (Location) can't
  // improve on the applicant's end — the change is Guavo expanding its
  // footprint — so folding "operations in a state we currently serve" into
  // "improve materially over time" reads as if we're asking them to relocate.
  // Those contribute a standalone paired note instead.
  const improvableThresholds = reasons
    .filter(r => typeof r.pairedReapply !== 'function')
    .map(r => r.threshold())
    .filter(Boolean);
  const coverageNotes = reasons
    .filter(r => typeof r.pairedReapply === 'function')
    .map(r => r.pairedReapply())
    .filter(Boolean);
  const thresholdClause = joinThresholds(improvableThresholds);
  const coverageClause = joinThresholds(coverageNotes);
  ...
  if (reasons.length === 1 && typeof reasons[0].soloReapply === 'function') {
    ...
    reapplyInvitation += coverageClause ? `, and ${coverageClause}.` : '.';
  } else if (coverageClause) {
    reapplyInvitation = `If the factors above change over time, please reapply. ${capitalizeFirst(coverageClause)}.`;
  } else {
